...
const SESSION_COOKIE_HINT =
  "请粘贴完整 Session JSON（含 sessionToken 字段，超长会自动分块），或从浏览器导出 chatgpt.com 的 cookies[]（含 __Secure-next-auth.session-token.0/.1）";
...
function shouldBlockLoginNavigation(url, resourceType = "") {
  ...
  if (
    ...
    isHardLoginRedirectUrl(url) &&
    !isCheckoutPageUrl(url)
  ) {
    return true;
  }
  return false;
...
async function isLoginPageContent(page) {
  try {
    ...
    return (
      /Sign in with Google/i.test(bodyText) ||
...
async function hasVisibleLoginChrome(page) {
  ...
  const probes = [
    ...
    () =>
      page
        .getByRole("link", { name: /^Sign up for free$/i })
        .first()
        .isVisible({ timeout: 800 }),
    () =>
      page
        .getByText(/^Sign up for free$/i)
        .first()
        .isVisible({ timeout: 800 }),
    () =>
      page
...
async function isWorkspacePickerVisible(page) {
  if (!page || (typeof page.isClosed === "function" && page.isClosed())) {
    return false;
  }
  const probes = [
    () =>
      page
        .getByText(/Launch a workspace/i)
        .first()
        .isVisible({ timeout: 800 }),
    () =>
      page
        .getByText(/Personal workspace/i)
        .first()
        .isVisible({ timeout: 800 }),
    () =>
      page
...
async function openPersonalWorkspace(page) {
  if (!(await isWorkspacePickerVisible(page))) {
    return false;
  }
  console.log("[ChatGPT] 检测到工作区选择页，正在打开个人工作区...");
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (await openBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const card = page.getByText(/Personal workspace/i).first();
    ...
    const gone = await page
      ...
      .then(() => true)
      .catch(() => false);
    if (gone || !(await isWorkspacePickerVisible(page))) {
      console.log("[ChatGPT] 已打开个人工作区");
      return true;
    }
    console.warn(`[ChatGPT] 工作区 Open 未离开选择页 (${attempt}/3)`);
...
async function hasLoggedInChatUi(page) {
  ...
  const probes = [
    ...
    () =>
      page
        .getByText(/Ready when you are/i)
        .first()
        .isVisible({ timeout: 1500 }),
    () =>
      page
        .getByText(/Ask anything/i)
        .first()
        .isVisible({ timeout: 1500 }),
    () =>
      page
        .locator(
          'textarea[placeholder*="Ask"], textarea#prompt-textarea, div[contenteditable="true"]',
        )
        .first()
        .isVisible({ timeout: 1500 }),
    ...
    () =>
      page
        .getByText(/Configure your plan/i)
        .first()
        .isVisible({ timeout: 1500 }),
    () =>
      page
        .getByText(/^Card number$/i)
        .first()
        .isVisible({ timeout: 1500 }),
    () =>
      page
        .getByText(/Due today/i)
        .first()
        .isVisible({ timeout: 1500 }),
  ];
  for (const probe of probes) {
...
async function isCheckoutLoginGate(page) {
  ...
  try {
    return await page
      .getByText(/Sign in to continue|Log in to continue|登录后继续/i)
      .first()
      .isVisible({ timeout: 800 });
  } catch (_) {
    return false;
