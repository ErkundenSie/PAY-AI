...
const SKIP_REGION_BUTTON_TEXT =
  /^(Upgrade|Personal|Business|Free|Plus|Pro|Subscribe|Close|Your current plan|升级|订阅|关闭)$/i;
...
async function isBusinessTabActive(page) {
  ...
  if (await businessTab.isVisible({ timeout: 800 }).catch(() => false)) {
    const selected = await businessTab
      .getAttribute("aria-selected")
      .catch(() => null);
    const state = await businessTab
  ...
  const businessCard = await page
    .getByText(/ChatGPT Business/i)
    .first()
    .isVisible({ timeout: 800 })
    .catch(() => false);
  const plusCard = await page
...
async function isPricingModalVisible(page) {
...
async function openPricingModalFromChat(page) {
  const candidates = [
    () =>
      page
        .getByRole("button", {
          name: /Upgrade plan|Upgrade your plan|Get Plus|Upgrade to Plus|升级套餐|升级计划|获取 Plus/i,
        })
        .first(),
    () =>
      page
        .locator("button")
        .filter({
          hasText: /Upgrade plan|Get Plus|Upgrade to Plus|升级套餐|获取 Plus/i,
        })
        .first(),
    () =>
      page
        .getByRole("menuitem", {
          name: /Upgrade plan|Get Plus|Upgrade to Plus|升级套餐/i,
        })
        .first(),
    () =>
      page.locator('[data-testid*="upgrade"], [aria-label*="Upgrade"]').first(),
  ];
  ...
  for (const getLocator of candidates) {
    try {
      ...
      if (await el.isVisible({ timeout: 1500 }).catch(() => false)) {
        ...
        if (await isPricingModalVisible(page)) {
          console.log("✅ [步骤] 已从聊天页打开套餐弹窗");
          return true;
        }
      }
    } catch (_) {
  ...
  const profileCandidates = [
    () =>
      page
        .getByRole("button", { name: /profile|account|user menu|账户|头像/i })
        .first(),
    () => page.locator('button[aria-haspopup="menu"]').last(),
    () => page.locator("nav button").last(),
  ];
  for (const getLocator of profileCandidates) {
    try {
      const el = getLocator();
      ...
      const upgradeItem = page
        .getByRole("menuitem", { name: /Upgrade|Get Plus|升级/i })
        .first();
      if (await upgradeItem.isVisible({ timeout: 1200 }).catch(() => false)) {
        await upgradeItem.click({ timeout: 8000 });
        await page.waitForTimeout(1500);
        if (await isPricingModalVisible(page)) {
          console.log("✅ [步骤] 已从账户菜单打开套餐弹窗");
          return true;
        }
      }
    } catch (_) {
...
async function isBusinessPlanView(page) {
  const businessTitle = await page
    .getByText(/ChatGPT Business/i)
    .first()
    .isVisible({ timeout: 800 })
    .catch(() => false);
...
async function switchToPersonalPlans(page) {
  ...
  if (await isBusinessPlanView(page)) {
    throw new Error(
      "定价页停留在 Business 套餐，未能切换到 Personal，无法购买 Plus/Pro",
    );
  }
...
async function readPricingSurfaceText(page) {
  ...
  return String(
    (await surface.innerText({ timeout: 5000 }).catch(() => "")) || "",
  );
}
...
const ALL_COUNTRY_NAME_PATTERN =
  /^(United Kingdom|Philippines|United States|Singapore|Malaysia|Afghanistan|Algeria|Andorra|Albania|Australia|Canada|Japan|China|菲律宾)$/i;
...
async function openRegionPicker(page) {
  ...
  const triggerCandidates = [
    ...
    surface
      ...
      .filter({ hasText: ALL_COUNTRY_NAME_PATTERN })
      .last(),
    surface
  ...
  for (const trigger of triggerCandidates) {
    try {
      ...
      const inListbox = await trigger
        .evaluate((node) => Boolean(node.closest('[role="listbox"]')))
        .catch(() => false);
      if (inListbox) {
        continue;
      }
  ...
  return viewport
    .evaluate((root, labelList) => {
      ...
      const elements = Array.from(
        root.querySelectorAll('[role="option"], [data-index], button, li, div'),
      );
      ...
      for (const el of elements) {
        ...
        const childHasSame = Array.from(el.children).some((child) => {
          const childLines = String(child.innerText || "")
            .split("\n")
            .map((line) => line.trim())
            .filter(Boolean);
...
async function scrollVirtualCountryList(page, labels) {
  ...
  const jumpRatio = targets.some((label) =>
    /philippines|pilipinas|菲律宾/i.test(label),
  )
    ? 0.58
    : 0.5;
  await viewport
  ...
  for (let step = 0; step < 160; step += 1) {
    ...
    const visibleTexts = await viewport
      .evaluate((root) => {
    ...
    if (step === 0 || step % 15 === 0) {
      console.log(
        `[Info] 滚动第 ${step + 1} 步，可见: ${visibleTexts.join(" | ") || "(empty)"}`,
      );
    }

    const atBottom = await viewport
      .evaluate((node) => {
...
async function readSelectedCountryLabel(page) {
  ...
  const trigger = surface
    ...
    .filter({
      hasText:
        /Philippines|United Kingdom|Algeria|United States|Singapore|Malaysia|Afghanistan|菲律宾/i,
    })
    .last();
...
async function selectRegionOption(page, regionCode) {
  ...
  const search = page
    .locator(
      'input[type="search"], input[placeholder*="Search" i], input[placeholder*="搜索" i]',
    )
    .first();
...
async function verifySelectedCountry(page, labels) {
  ...
  if (selected) {
    console.warn(
      `[Warn] 地区选择校验失败，当前显示: ${selected}，期望: ${labels.join(" / ")}`,
    );
    return false;
  }
  return true;
...
async function waitForPricingPage(page, timeout = 60000) {
  ...
  if (await openPricingModalFromChat(page)) {
    return;
  }

  const surfacePreview = (await readPricingSurfaceText(page))
    .replace(/\s+/g, " ")
    .slice(0, 120);
  throw new Error(
    `定价页未弹出套餐弹窗，当前仍是聊天页。URL: ${lastUrl} 页面: ${surfacePreview}`,
  );
}
...
async function selectPricingRegion(page, regionCode) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (menuOpen || (await isRegionMenuOpen(page))) {
      ...
      if (!selected) {
        console.warn(
          `[Warn] 地区菜单已打开，但未找到 ${code} 对应选项（将尝试滚动列表）`,
        );
      } else {
  ...
  const finalText = (await readPricingSurfaceText(page))
    .replace(/\s+/g, " ")
    .slice(0, 160);
  throw new Error(
    `无法将定价页切换到目标地区 ${code}（${regionConfig?.label || code}），请检查后台支付地区设置。当前页面: ${finalText}`,
  );
}
...
async function clickPlanUpgrade(page, planType) {
  ...
  if (await isBusinessPlanView(page)) {
    throw new Error(
      `定价页仍在 Business 视图，未找到 ${plan} 套餐；请确认账号可升级 Plus/Pro`,
    );
  }

  throw new Error(
    `未找到 ${plan} 套餐的升级按钮，请确认账号可升级且 Session 已登录`,
  );
}
