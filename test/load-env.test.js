...
afterEach(() => {
    for (const temporaryPath of temporaryPaths.splice(0)) fs.rmSync(temporaryPath, { force: true });
});

describe('loadEnvFile', () => {
    it('loads .env values without replacing explicit environment values', () => {
        const temporaryPath = path.join(os.tmpdir(), `kc-pay-env-${Date.now()}-${Math.random()}.env`);
        temporaryPaths.push(temporaryPath);
        fs.writeFileSync(temporaryPath, 'DB_HOST=db.example.com\nDB_PASSWORD="p@ss # word"\nPROXY=http://user:pass@example.com # note\n');
